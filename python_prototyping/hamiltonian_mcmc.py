...
def hamiltonian_mcmc(q_init, data, num_samples):
    samples = []
    mass = 0.01 * np.ones(shape=(len(q_init)), dtype=float)
...
def draw_momentum(mass_matrix):
    ...
    p_proposed = np.random.multivariate_normal(np.zeros(shape=(np.shape(mass_matrix)[0])), mass_matrix)
...
def accept_state_rule(q, q_proposed, p, p_proposed, U, K):
    ...
    #### remove
    if math.isnan(accept_probability):
        return False
...
num_samples = 100000
samples = hamiltonian_mcmc(q, observation, num_samples)
